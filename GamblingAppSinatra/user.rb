...
post '/logout' do
  ...
  @user.update(totalWinnings: updateWinnings.to_i, totalLosings: updateLosings, totalProfit: updateProfit)
  session.clear
  erb :login
end
...
post '/gambleUI' do
  ...
  if(@user == nil)
    @user = User.create(username: "#{username}", password: "#{password}", totalWinnings: 0, totalLosings: 0, totalProfit: 0)
  end
...
def save_session(winnings, losings)
  ...
  session[:totalProfit] = (session[:totalWinnings].to_i) - (session[:totalLosings].to_i)
end
